...
export default function TLUnpaidPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data } = await supabase
        .from('sales_records')
        .select('id, smartcard_number, serial_number, customer_name, sale_date, package_status, dsr_id')
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-display font-bold">TL Unpaid</h1>
          <p className="text-muted-foreground mt-1">Unpaid sales for {currentTeamLeader?.name || 'your account'} • {salesDateLabel}</p>
        </div>
        ...
        <GlassCard>
          <div className="overflow-x-auto">
            {loading ? <div className="space-y-4 p-4">{[...Array(5)].map((_, index) => <Skeleton key={index} className="h-12 w-full" />)}</div> : (
              <Table>
                <TableHeader><TableRow><TableHead>Smartcard</TableHead><TableHead>Serial</TableHead><TableHead>Customer</TableHead><TableHead>Date</TableHead><TableHead>Package</TableHead><TableHead>Completion</TableHead></TableRow></TableHeader>
                <TableBody>
                  {filteredSales.length === 0 ? <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No unpaid sales found.</TableCell></TableRow> : filteredSales.map((sale) => (
                    <TableRow key={sale.id}><TableCell className="font-mono text-sm">{sale.smartcard_number}</TableCell><TableCell className="font-mono text-sm">{sale.serial_number}</TableCell><TableCell>{sale.customer_name || '-'}</TableCell><TableCell>{sale.sale_date}</TableCell><TableCell><Badge variant="outline">{sale.package_status}</Badge></TableCell><TableCell><Badge className={getSaleCompletionBadgeClass(sale.dsr_id)}>{getSaleCompletionLabel(sale.dsr_id)}</Badge></TableCell></TableRow>
                  ))}
                </TableBody>
              </Table>
            )}
        ...
        <GlassCard className="p-4 text-center"><CreditCard className="h-6 w-6 mx-auto text-red-500 mb-2" /><p className="text-2xl font-bold">{filteredSales.length}</p><p className="text-sm text-muted-foreground">Unpaid Records</p></GlassCard>
      </div>
    </AdminLayout>
  );
}